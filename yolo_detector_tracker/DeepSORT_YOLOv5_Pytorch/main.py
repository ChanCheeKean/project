    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                cv2.circle(img0, (int(center[0]), int(center[1])), 3, (0, 0, 255), -1)
            ...
            cv2.line(img0, (left_line_position, ih // 2), (left_line_position, ih), (255, 0, 0), 2)
            cv2.line(img0, (right_line_position, ih // 2), (right_line_position, ih), (255, 0, 0), 2)
            cv2.line(img0, (left_line_position, ih // 2), (right_line_position, ih // 2), (255, 0, 0), 2)
            img0 = cv2.flip(img0, 1)
            text_scale = max(1, img0.shape[1] // 1600)
            cv2.putText(img0, f"In -->: {left_count}", (20, 40), font_type, font_size, font_color, font_thickness)
            cv2.putText(img0, f"Out <--: {right_count}", (20, 80), font_type, font_size, font_color, font_thickness)
            cv2.putText(img0, 'frame: %d fps: %.2f ' % (idx_frame, len(avg_fps) / sum(avg_fps)),
                        (iw - iw // 8, 20 + text_scale), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255), thickness=2)
            
            # if include_count > 1:
            #     cv2.putText(img0, f"ALARM", (ih // 2 - 50, iw // 2 - 150), font_type, 8, (0, 0, 255), 5)
